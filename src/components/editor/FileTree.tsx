...
function FileTreeNode({ node, level }: FileTreeNodeProps) {
  ...
  return (
    <div>
      <div
        className={cn(
          "flex items-center gap-2 px-2 py-1.5 hover:bg-slate-800 cursor-pointer text-sm transition-colors rounded-sm mx-1",
      ...
      >
        ...
        <span className={cn("truncate", selectedFile === node.path ? "text-slate-100 font-medium" : "text-slate-300")}>{node.name}</span>
      </div>
  ...
  if (!rootNode || !rootNode.children || rootNode.children.size === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 text-center">
        ...
        <p className="text-xs text-slate-600 mt-1">Files will appear here</p>
      </div>
    );
  }
